...
- **[ACTIVE] 1080p needs the REST API, not the MCP.** `mcp__heygen__generate_avatar_video` outputs **720p** (no dimension param). For golden-standard 1080p use `POST v2/video/generate` with `"dimension":{"width":1920,"height":1080}` (key: `~/ecosystem/vault/secrets.env` `HEYGEN_API_KEY`, header `X-Api-Key`).
