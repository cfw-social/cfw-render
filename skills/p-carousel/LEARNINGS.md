...
- `render.mjs` defaults to `SCALE=2` (2160×2700) but `acceptance.json` `canvas_1080x1350` is an EXACT dims check → every slide FAILs the gate until downscaled. Headless runs: render at 2× for sharpness, then `scale=1080:1350:flags=lanczos` into `slides/` (keep the 2× set in `slides-2x/`).
